...
| M1–M6 | 🟡 | method | The collapse diagnostic can't detect collapse; the 16.8M-fact regime; **E9 has no effect in the production trainer (M3 ✅ reproduced)**; the dense-warmup results depend on C1–C2; the unit test can't fail | code + M3 reproduced |
...
| M3 | **The E9 "coverage collapse eliminated" result** (DOC-08, commit `30d854a`) doesn't come from E9. ✅ **Reproduced with the trainer's exact pattern** (tables captured by the jitted step, refreshed by reassigning `usage_box[0]`):
<br>• With the initial zero tables, E9 gives the **same loss and gradients as E9 off** (loss 5.833681 in both; max gradient difference 1.9e-6, which is float noise).
<br>• After a refresh, the jitted step **still returns the zero-table loss** (5.833681; the loss with the new tables would be 5.833584).
<br>• The same commit also turned on `lb_weight=0.02` (previously 0) and changed the coverage parser. So the jump from 1.3% to 18% coverage is most likely the entropy loss (C7) plus the metric change, and the entropy loss spreads coverage by flattening routing. Validation bpc did not improve (9.7085 vs 9.7009).
| Fix C1 and C3, then run a 3-arm comparison with the same metric code: (a) neither, (b) `lb_weight` only, (c) E9 only |
